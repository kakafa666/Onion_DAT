...
class ModelLightningModule(MatglLightningModuleMixin, pl.LightningModule):
    ...
    def step(self, batch: tuple):
        ...
        if isinstance(self.model, M3GNet):
            ...
            u,gap_pred = self(g=g, l_g=l_g, state_attr=state_attr)
        else:
            g, labels, state_attr = batch # labels是真值
            u,gap_pred = self(g=g, state_attr=state_attr)
            if gap_pred.dim() == 1:
                gap_pred = torch.unsqueeze(gap_pred, dim=0)
                u = torch.unsqueeze(u, dim=0)

            delta = torch.abs(labels - gap_pred[:, 0]) # |Ptrue - P|
            labels = torch.stack((delta, labels), dim=1)

        results = self.loss_fn(loss=self.loss, preds=(u,gap_pred), labels=labels)
        batch_size = gap_pred.numel()
    ...
    def loss_fn(self, loss: nn.Module, labels: torch.Tensor, preds: torch.Tensor):
        ...
        # loss 默认为mse zdn
        u,gap_pred = preds
        diff = torch.abs(labels[:, 1] - gap_pred[:, 0])# |Ptrue - P|
        u_loss = loss(diff, u[:, 0])# (|Ptrue - P| - u)²
        total_loss = loss(labels[:, 1], gap_pred[:, 0]) + u_loss # 计算总损失 (Ptrue -P)² + (|Ptrue - P| - u)²
        mae = self.mae(labels[:, 1], gap_pred[:, 0]) + self.mae(labels[:, 0], u[:, 0])
        rmse = self.rmse(labels[:, 1], gap_pred[:, 0]) + self.rmse(labels[:, 0], u[:, 0])
