...
def get_train_val_test_dataset():
    ...
    # 高保真度数据集
    Exp_structures = [Structure.from_dict(x['structure']) for x in d['ordered_exp'].values()]  # 将文件中的原始结构数据转换为 pymatgen 结构
    ...
    # 使用带隙离散化后的标签进行分层抽样，划分训练集和测试集
    exp_train_test_data, exp_test_data = train_test_split(combined,
                                                test_size=0.5,
                                                random_state=42,
                                                stratify=targets_discretized)

    # 提取训练集和测试集的索引
    train_test_indices = np.array([i for i in range(n_exp) if combined[i] in exp_train_test_data])
    test_indices = np.array([i for i in range(n_exp) if combined[i] in exp_test_data])

    # 将训练集再划分为训练集和验证集
    exp_train_data, exp_val_data = train_test_split(exp_train_test_data,
                                            test_size=0.2,
                                            random_state=42,
                                            stratify=[targets_discretized[i] for i in train_test_indices])

    # 获取验证集的索引
    val_indices = np.array([i for i in train_test_indices if combined[i] in exp_val_data])
    ...
    # 提取训练集的化学结构和目标带隙数据
    exp_train_structures = [Exp_structures[i] for i in train_test_indices if combined[i] in exp_train_data]
    exp_train_targets = [Exp_targets[i] for i in train_test_indices if combined[i] in exp_train_data]

    # print(f"验证集大小：{len(exp_val_data)}")
    # print(f"测试集大小：{len(exp_test_data)}")
    # print(f"训练集大小：{len(exp_train_data)}")

    return exp_train_structures, exp_train_targets, exp_val_structures, exp_val_targets, exp_test_structures, exp_test_targets
