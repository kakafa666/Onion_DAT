...
def training_model(train_inputs,train_outputs,saved_name):
    ...
    # 检查是否存在模型检查点
    checkpoint_path = f"./saved_models/{saved_name}/epoch=233_val_MAE=0.326.ckpt"
    if os.path.exists(checkpoint_path):
        print(f"Loading checkpoint from {checkpoint_path}")
        # 加载模型检查点
        model = Cgcnn_lightning.load_from_checkpoint(checkpoint_path, crystalGraphConvNet=model.crystalGraphConvNet,
    ...
    # 设置早停回调的最佳分数
    early_stop_callback.best_score = torch.tensor(0.326)
    ...
    trainer = pl.Trainer(max_epochs=100000, callbacks=[early_stop_callback, checkpoint_callback,normalizer_callback], logger=logger,accelerator="cpu")
