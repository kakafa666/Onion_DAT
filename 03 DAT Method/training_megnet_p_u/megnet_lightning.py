...
class MatglLightningModuleMixin:
    ...
    def training_step(self, batch: tuple, batch_idx: int):
        ...
        if len(regression_data) > 0:
            ...
            gap_pred,u= self.regressor(regression_features)
            if gap_pred.dim() == 1:
                gap_pred = torch.unsqueeze(gap_pred, dim=0)
                u = torch.unsqueeze(u, dim=0)

            delta = torch.abs(regression_labels - gap_pred[:, 0])  # |Ptrue - P|
            p_loss = nn.MSELoss()(regression_labels, gap_pred)  # |Ptrue - P|
            u_loss = nn.MSELoss()(delta, u)  # (|Ptrue - P| - u)²
    ...
    def validation_step(self, batch: tuple, batch_idx: int):
        ...
        if len(regression_data) > 0:
            ...
            # delta = torch.abs(regression_labels - gap_pred[:, 0])  # |Ptrue - P|
            delta = torch.abs(regression_labels - gap_pred)    # |Ptrue - P|
            p_loss = nn.MSELoss()(regression_labels, gap_pred)  # |Ptrue - P|
            u_loss = nn.MSELoss()(delta, u)  # (|Ptrue - P| - u)²
